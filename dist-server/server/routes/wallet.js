...
export default function walletRoutes(pool) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        ...
        if (!userId)
            return;
        try {
            const result = await pool.query(`SELECT id, address, confirmed, created_at FROM wallets WHERE user_id = $1`, [userId]);
    ...
    router.post('/create', async (req, res) => {
        ...
        if (!userId)
            return;
        try {
            const existing = await pool.query('SELECT id FROM wallets WHERE user_id = $1', [userId]);
            ...
            await pool.query(`INSERT INTO wallets (user_id, address, encrypted_private_key, confirmed)
         VALUES ($1, $2, $3, FALSE)`, [userId, address, encryptedKey]);
            res.status(201).json({
                address,
    ...
    router.get('/balance', async (req, res) => {
        ...
        if (!userId)
            return;
        try {
            const walletResult = await pool.query('SELECT address FROM wallets WHERE user_id = $1 AND confirmed = TRUE', [userId]);
    ...
    router.post('/confirm', async (req, res) => {
        ...
        if (!userId)
            return;
        try {
            const result = await pool.query(`UPDATE wallets SET confirmed = TRUE WHERE user_id = $1 AND confirmed = FALSE
         RETURNING id, address, confirmed`, [userId]);
            if (result.rows.length === 0) {
