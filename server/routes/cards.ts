...
function generateLuhnValidVisa(): string {
  const prefix = '4';
  let digits = prefix;
  for (let i = 1; i < 15; i++) {
...
export default function cardRoutes(pool: pg.Pool) {
  const router = Router();
  ...
  router.get('/:id/details', async (req: Request, res: Response) => {
    ...
    try {
      const result = await pool.query(
        `SELECT id, card_number, cvv, expiry, name, frozen, online_payments, contactless, created_at
  ...
  router.patch('/:id/freeze', async (req: Request, res: Response) => {
    const userId = requireAuth(req, res);
    if (!userId) return;

    try {
      const result = await pool.query(
        `UPDATE cards SET frozen = TRUE WHERE id = $1 AND user_id = $2 RETURNING id, frozen`,
        [req.params.id, userId]
      );
  ...
  router.patch('/:id/unfreeze', async (req: Request, res: Response) => {
    const userId = requireAuth(req, res);
    if (!userId) return;

    try {
      const result = await pool.query(
        `UPDATE cards SET frozen = FALSE WHERE id = $1 AND user_id = $2 RETURNING id, frozen`,
        [req.params.id, userId]
      );
  ...
  router.patch('/:id/toggle', async (req: Request, res: Response) => {
    ...
    try {
      const result = await pool.query(
        `UPDATE cards SET ${field} = NOT ${field} WHERE id = $1 AND user_id = $2
         RETURNING id, frozen, online_payments, contactless`,
        [req.params.id, userId]
      );
